package fred.monstermod.raid.core;

public class RaidConfig {
    public static final String WORLD_NAME = "raid_world";

    public static final int START_X_SPREAD = 8000000;
    public static final int START_Z_SPREAD = 8000000;

    public static final int END_X_SPREAD = 250;
    public static final int END_Z_SPREAD = 250;

    public static final int RAID_TIME_LIMIT_MINUTES = 25;
    public static final int RAID_TIME_ANNOUNCE_EVERY_MINUTE_BEFORE = 5;

    public static final String COMPASS_ITEM_NAME = "Raid Exit Compass";
    public static final int EXTRACTION_DISTANCE_MAX = 20;

    public static final int MAX_BLOCKS_TO_BREAK_PER_RAID = 100;
    public static final int BLOCKS_TO_BREAK_WARNING_EVERY_X_BLOCKS = 5;

    public static final int REQUIRED_ONLINE_PLAYERS = 2;
}
